...
function loneLines(el: HTMLElement): number {
  const words: { top: number; seg: number }[] = [];
  let seg = 0;
  ...
  while ((n = walk.nextNode())) {
    ...
    while ((m = re.exec(t.data))) {
      ...
      if (rc && rc.width) words.push({ top: rc.top, seg });
    }
  }
  const perSeg = new Map<number, number>();
  words.forEach((w) => perSeg.set(w.seg, (perSeg.get(w.seg) || 0) + 1));
  const lines: { seg: number; top: number; n: number }[] = [];
  for (const w of words) {
    const line = lines.find((l) => l.seg === w.seg && Math.abs(l.top - w.top) < 6);
    if (line) line.n++;
    else lines.push({ seg: w.seg, top: w.top, n: 1 });
  }
...
function wireFit() {
  ...
  fitWired = true;
  let t = 0;
  const later = () => { clearTimeout(t); t = window.setTimeout(fitHeadings, 120); };
  fitHeadings();
